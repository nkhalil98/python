...
def run_threaded_workers(worker_type: WorkerType, num_workers: int = 10) -> list[int]:
    data = []
    worker = io_worker if worker_type == "io" else computing_worker
    ...
    print(f"Total time: {end - start:.2f} seconds (threading)")
...
def run_thread_pool_executor(
    ...
    print(
        f"Total time: {end - start:.2f} seconds (ThreadPoolExecutor) with {MAX_WORKERS} workers"
    )
...
if __name__ == "__main__":
    ...
    # threading code
    run_threaded_workers("io")

    # ThreadPoolExecutor code
    run_thread_pool_executor("io")
